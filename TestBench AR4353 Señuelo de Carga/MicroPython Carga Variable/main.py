# Este es el código final para la configuración de la carga variable
...
Description: 
    ...
    -This code only focus in the read load data of the power analyzer.
    -The data is stored in a dictionary and printed on the screen.
    -The data is read in one core1 and the data from the power analyzer in another core.

========================================
COMANDOS JSON DE ENTRADA SOPORTADOS
========================================

1. CONFIGURACIÓN MODO LIST:
   {"Funcion": "LIST", "Config": {"Resolution": [10, 0.2, 0.5, "STEP1"], "Range": [50, 3]}, "Start": "CFG_ON"}
   
   Parámetros Resolution:
   - [0]: Número de pasos (int)
...
2. CONFIGURACIÓN MODO DYN (Dinámico):
   {"Funcion": "DYN", "Config": {"Resolution": [0.350, 0.20, 0.350, 0.20], "Range": [50, 3]}, "Start": "CFG_ON"}
   
   Parámetros Resolution:
   - [0]: Nivel A dinámico (float)
   - [1]: Ancho A dinámico (float)
   - [2]: Nivel B dinámico (float)
   - [3]: Ancho B dinámico (float)

3. CONFIGURACIÓN IDIOMA:
...
========================================
RESPUESTAS JSON QUE ENVÍA EL SISTEMA
========================================

1. Respuesta de configuración:
...
DEBUG = False  # Cambiar a False para desactivar SOLO los mensajes de debug
              # Las respuestas JSON SIEMPRE se envían (son datos importantes)
# ==== FIN CONFIGURACIÓN DEBUG ====

def debug_print(message):
    """Función para imprimir mensajes de debug solo si DEBUG está activado
    NOTA: Las respuestas JSON se envían SIEMPRE, independientemente de esta bandera"""
    if DEBUG:
        print(message)
...
# UART para comunicar con la carga variable
uart1 = machine.UART(1, baudrate=9600, tx=machine.Pin(TX_PIN, machine.Pin.OUT), rx=machine.Pin(RX_PIN, machine.Pin.IN))

# UART para comunicar con la PagWeb
uart2 = machine.UART(2, baudrate=9600, tx=machine.Pin(TX_PIN2, machine.Pin.OUT), rx=machine.Pin(RX_PIN2, machine.Pin.IN))
...
def readSerialUART2():
    global uart2_buffer
    while uart2.any():
        uart2_buffer += uart2.read(1)  # leer byte por byte
        if b"\n" in uart2_buffer:      # detecta línea completa
            line, uart2_buffer = uart2_buffer.split(b"\n", 1)
            try:
                text = line.decode().strip()
                if text:
                    _command = ujson.loads(text)
                    if all(k in _command for k in ("Funcion","Config","Start")):
                        print("DEBUG: JSON recibido correctamente:", _command)
                        return _command
                    else:
                        print("JSON incompleto:", _command)
            except Exception as e:
                print("Error parseando JSON:", e, line)
    return None
...
def readSerial():
    try:
        ...
        debug_print(f"DEBUG: Datos problemáticos: {data}")
        return None
    except Exception as e:
...
def start_load(_command):
    ...
    if _command == "CFG_ON":
        ...
        uart1.write("INP 1\n") # Envío de comando a UART de CargaVar
        data = {
        "CONF": _command,
        "muestras": None,
        }
...
def config_load(command, d,ranger):
    ...
    if command == "LIST":
        ...
        uart1.write("FUNC LIST\n")							# Envío de comando a UART de CargaVar
        uart1.write("CURR:RANG "+str(ranger[0])+"\n")		# Envío de comando a UART de CargaVar
        uart1.write("LIST CLEAR\n")							# Envío de comando a UART de CargaVar
        uart1.write("LIST:FILE "+str(d[3])+"\n")			# Envío de comando a UART de CargaVar
        for item in data_for_steps:
            uart1.write("LIST ADD\n") 						# Envío de comando a UART de CargaVar
            uart1.write("LIST:LEV:" + str(item)+"\n")		# Envío de comando a UART de CargaVar
            debug_print(f"DEBUG: LIST:LEV: {item}")
        uart1.write( "LIST DONE\n")							# Envío de comando a UART de CargaVar
        uart1.write("LIST:MODE CONT\n")						# Envío de comando a UART de CargaVar
        debug_print("DEBUG: Configuración LIST completada")
        return True
    
    if command == "DYN":
        debug_print("DEBUG: Configurando modo DYN")
        uart1.write("FUNC DYN\n")
        uart1.write("CURR:RANG "+str(ranger[0])+"\n")
        uart1.write("VOLT:RANG "+str(ranger[1])+"\n")
        uart1.write("DYN:ALEV "+str(d[0])+"\n")
        uart1.write("DYN:AWID "+str(d[1])+"\n")
        uart1.write("DYN:BLEV "+str(d[2])+"\n")
        uart1.write("DYN:BWID "+str(d[3])+"\n")
        debug_print("DEBUG: Configuración DYN completada")
        
        return True
    
    if command == "LANG":
        debug_print("DEBUG: Configurando idioma")
        uart1.write("SYS:LANG EN\n")
        uart1.write("SYS:VOIC 0\n")
...
#             print("mas te vale funcionar ")
            ...
            # Envío de respuesta JSON - SIEMPRE se envía (independiente de DEBUG)
            debug_print(f"DEBUG: Enviando JSON por stdout: {json_data}")
            sys.stdout.buffer.write(str(json_data)+ '\n')  # ← RESPUESTA JSON SIEMPRE SE ENVÍA
            #uart2.write(str(json_data)+ '\n')            # Esta linea es la comentada
            flag_print = False
        else:
            #result = readSerial()
            result = readSerialUART2() #Lee JSON por UART2 PagWeb
            ...
            if result is not None:
                ...
                try:
                    if result.get('Funcion') != "Other":
                        debug_print(f"DEBUG: Ejecutando función de configuración: {result['Funcion']}")
                        ...
                        json_data = ujson.dumps(data)
                        debug_print(f"DEBUG: Respuesta de configuración: {json_data}")
                        flag_print = True
...
def CORE2():
    ...
    while True:
        ...
        if flag_finish == True:
            ...
            while cont +1< num_mues: # le puse el +1
                t0 = time.ticks_us()
                debug_print(f"DEBUG: CORE2 - Solicitando medición {cont+1}/{num_mues}")
                #uart2.write("MEAS:VOLT?;MEAS:CURR?;MEAS:POW?\n")
                uart1.write("MEAS:VOLT?;MEAS:CURR?;MEAS:POW?\n")
                time.sleep(0.3)
                # este codigo es para leer el sensor de potencia

                if uart1.any():
                    data = uart1.read()                    
                    print("El UART1 tiene algo")
                    
                    if data:  
                        datos = data.decode()
                        JSON_med = {"medicion": datos}
                        json_str = ujson.dumps(JSON_med)
                        uart2.write(json_str + "\n") 
                        
                        print("Envio el dato como JSON:", json_str)
                
                    # Código original String -> Pulsar -> String
                        '''
                        if uart1.any():
                            data = ""
                            data = uart1.read()
                            ...
                            datos = data.decode()
                            uart2.write(data) # Envio de medición a PC
                            print("Envio el dato")
                        '''
                    
                    else:
                        # meas[cont]= '0.0,0.0,0.0'
                        #json_str = ujson.dumps({"medicion": 0})
                        #uart2.write(json_str + "\n") 
                        
                        print("Envio de JSON vacío:")
                ...
                cont = cont+1
                tf = time.ticks_us()
                debug_print(f"DEBUG: CORE2 - Tiempo de medición: {tf - t0} us")
