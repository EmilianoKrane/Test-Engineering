...
// Estados para DipSwitch
const [stateDIP1, setDIP1On] = useState("disp");
const [stateDIP2, setDIP2On] = useState("disp");
const [stateDIP3, setDIP3On] = useState("disp");
const [stateDIP4, setDIP4On] = useState("disp");

// Estados para LEDS
const [ledStates, setLedStates] = useState<string[]>(["disp", "disp", "disp", "disp", "disp"]);
const ledLabels = ["5V", "9V", "12V", "15V", "20V"]; 

const [ledStates33, setLedStates33] = useState<string[]>(["disp", "disp", "disp", "disp", "disp"]);
...
const dipStates = [stateDIP1, stateDIP2, stateDIP3, stateDIP4];
...
const resetValores = () => {
    setDIP1On("disp");       // Espera DipSwitch posición
    setDIP2On("disp");      
    setDIP3On("disp");       
    setDIP4On("disp");       
}
...
const initValores = () => {
    setDIP1On("on");       // Espera DipSwitch posición
    setDIP2On("off");      
    setDIP3On("off");       
    setDIP4On("off");       
      ...
      for (const part of parts){
        ...
        try {
          ...
          if (parsed) {
            ...
            // Guardamos los valores de la clave de mediciones 
            if (parsed.medicion) setMedicion(parsed.medicion);
          }
        } 
        catch (err) {
          console.error("Error parseando JSON:", err, trimmed);
...
const verificarMedicion = async (): Promise<boolean> => {
  try {
    // Lectura inicial
    await delay(1200);
    await sendToLoad('{"Funcion": "Other", "Config": {"Resolution": [], "Range": []}, "Start": "Read"}');
...
const Medir33V = async (): Promise<boolean> => {
  try {
    ...
    await sendToLoad('{"Funcion": "DYN", "Config": {"Resolution": [1.5, 1, 1.5, 1], "Range": [50, 30]}, "Start": "CFG_ON"}');
    await delay(1200);
...
const Arranque = async () => {
    ...
    try {
      ...
      const exito = await enviarIDProducto(String(ID_Producto));
      //const exito = true; 
      ...
      // == PROCESO PARA 9V
      setDIP1On("off");    setDIP2On("off");    setDIP3On("off");    setDIP4On("off");          
      ...
      // == PROCESO PARA 12V
      setDIP1On("off");    setDIP2On("off");    setDIP3On("on");    setDIP4On("off");          
      ...
      // == PROCESO PARA 15V
      setDIP1On("off");    setDIP2On("on");    setDIP3On("on");    setDIP4On("off");          
      DipSwitch = await showMessageUntilSuccess(
      ...
      // == PROCESO PARA 20V
      setDIP1On("off");    setDIP2On("on");    setDIP3On("off");    setDIP4On("off");          
      DipSwitch = await showMessageUntilSuccess(
      ...
      await sendToLoad('{"Funcion": "DYN", "Config": {"Resolution": [5, 1, 5, 1], "Range": [50, 30]}, "Start": "CFG_ON"}');
      await delay(1200);
      console.log("➡️ Encendiendo la carga variable...");
      await sendToLoad('{"Funcion": "Other", "Config": {"Resolution": [], "Range": []}, "Start": "CFG_ON"}');
      await delay(2000);
      ...
      if (ledStates.every(state => state === "on") && 
          ledStates33.every(state => state === "on")){
        setProducto("Valido");
      }
      else{
...
const enviarNewTest = async (jsonData: Record<string, any>): Promise<boolean> => {
  ...
  return (
    ...
    <div
        ...
        >Coloca el DipSwitch en la posición indicada 🔎 </h3>
...
{dipStates.map((s, i) => {
    ...
    return (
        <div
            ...
            style={{
                ...
                background: isOn
                    ...
                    : isOff
                    ? "linear-gradient(to bottom, #ff7f50 0%, #e63946 100%)"
                    : "linear-gradient(to bottom, #ddd 0%, #aaa 100%)",
                boxShadow: `
        ...
        >
            {/* Palanca que sube o baja */}
            <div
                style={{
                    position: "absolute",
                    bottom: isOn ? "50px" : isOff ? "10px" : "50%",
                    left: "50%",
                    ...
                    borderRadius: "6px",
                    background: isOn 
                        ? "linear-gradient(to bottom, #a5d6a7 0%, #81c784 100%)"
                        : isOff
                        ? "linear-gradient(to bottom, #fd727eb7 0%, #fb4d4d 100%)"
                        : "linear-gradient(to bottom, #555 0%, #777 100%)",
                    boxShadow: `
            ...
            {/* Base del dipswitch - opcional texto ON/OFF abajo */}
            <div
                style={{
                    position: "absolute",
                    bottom: "-25px",
        ...
        style={{
            display: "flex",
            justifyContent: "center",
            marginTop: "20px" // separa del dipswitch
        }}
        >
        <button
            onClick={Arranque}
        ...
        {/* 
        <button
             onClick={() => window.dispatchEvent(new CustomEvent("clickCheck"))}
            style={{
                padding: "12px 0px",
                borderRadius: "6px",
                border: "none",
                backgroundColor: "#7326ef",
                color: "white",
                fontWeight: "bold",
                cursor: "pointer",
                boxShadow: "0 3px 6px rgba(0,0,0,0.3)",
                ...
                width: "100px",
                marginLeft: "30px",
                fontSize: "24px"  // Aumento de tamaño de letra
            }}
            onMouseOver={(e) => (e.currentTarget.style.backgroundColor = "#5a1dbb")}
            onMouseOut={(e) => (e.currentTarget.style.backgroundColor = "#7326ef")}
        >
            ¡Listo!
        </button>
        */}
        ...
        <div
          style={{
            display: "flex",
            flexDirection: "column", // coloca las filas una debajo de la otra
            justifyContent: "center",
            alignItems: "center", // centra ambas filas
                  ...
                  <div
                    style={{
                      ...
                      backgroundColor: isOn
                        ...
                        ? "#FFC107"
                        : "#e6e6e6ff",
                      boxShadow: `0 6px 6px rgba(0,0,0,0.3), inset 0 2px 3px rgba(255,255,255,0.3)`,
                      transition: "all 0.3s ease",
                    }}
                  />
                  {/* Leyenda */}
    ...
    >
        ...
        onClick={() => {
            ...
            setDIP1On("disp");          // Apaga DipSwitch
            setDIP2On("disp");          // Apaga DipSwitch
            setDIP3On("disp");          // Apaga DipSwitch
            setDIP4On("disp");          // Apaga DipSwitch
